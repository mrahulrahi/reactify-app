...
const initialState = {
   ...
   emailPassword: {},
   otpRes: {},
   profileNotVerfiedToken: null
};

export const createTrundlerProfile = createAsyncThunk(
   "auth/login",
   async ({ formData, access_token }) => {
         const config = {
         headers: {
...
export const setProfileNotVerifiedToken = createAsyncThunk(
   "auth/set_profile_not_verfied_token",
   async (data) => {
      return data;
   }
);
...
const loginSice = createSlice({
   ...
   extraReducers: (builder) => {
      ...
      builder.addCase(setProfileNotVerifiedToken.fulfilled, (state, { payload }) => {
         state.loading = false;
         state.profileNotVerfiedToken = payload;
         state.error = null;
      });
