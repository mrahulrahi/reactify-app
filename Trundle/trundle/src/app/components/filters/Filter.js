...
export default function Filter({ countriesList, fetchedItinerariesLength, priceList }) {
   ...
   const handlePriceChange = (event) => {
      ...
      if (parseInt(event?.price) === 0) {
         createQueryString("price", "free");
      } else {
         createQueryString("price", parseInt(event?.price));
      }
   };
